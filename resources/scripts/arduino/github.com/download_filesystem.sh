...
echo "HAL9000: Downloading littlefs version '$HAL9000_FIRMWARE_VERSION' for '$HAL9000_ARDUINO_VENDOR: $HAL9000_ARDUINO_PRODUCT'..."
...
if [ ! -f "${GIT_DIR}/resources/downloads/${HAL9000_PIO_NAME}_littlefs_${HAL9000_FIRMWARE_VERSION}.bin" ]; then
	wget -q --show-progress -O "${GIT_DIR}/resources/downloads/${HAL9000_PIO_NAME}_littlefs_${HAL9000_FIRMWARE_VERSION}.bin" \
	     "https://github.com/juergenpabel/HAL9000/releases/download/${HAL9000_FIRMWARE_VERSION}/${HAL9000_PIO_NAME}_littlefs.bin"
	if [ $? -ne 0 ]; then
