...
sudo sh -c 'echo "ATTRS{id}==\"'${ALSA_ID}'\",KERNEL==\"card?\",    RUN+=\"mkdir /dev/snd/HAL9000\""  >> /etc/udev/rules.d/99-hal9000-alsa.rules'
sudo sh -c 'echo "ATTRS{id}==\"'${ALSA_ID}'\",KERNEL==\"controlC?\",SYMLINK+=\"snd/HAL9000/%k\""      >> /etc/udev/rules.d/99-hal9000-alsa.rules'
sudo sh -c 'echo "ATTRS{id}==\"'${ALSA_ID}'\",KERNEL==\"pcmC?D?c\", SYMLINK+=\"snd/HAL9000/%k\""      >> /etc/udev/rules.d/99-hal9000-alsa.rules'
sudo sh -c 'echo "ATTRS{id}==\"'${ALSA_ID}'\",KERNEL==\"pcmC?D?p\", SYMLINK+=\"snd/HAL9000/%k\""      >> /etc/udev/rules.d/99-hal9000-alsa.rules'
