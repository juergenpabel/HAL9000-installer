...
echo "HAL9000: Downloading firmware version '${HAL9000_INSTALL_VERSION}' for '${HAL9000_ARDUINO_ID}'..."
GIT_DIR=`git rev-parse --show-toplevel`

if [ ! -f "${GIT_DIR}/resources/downloads/${HAL9000_ARDUINO_ID}_firmware_${HAL9000_INSTALL_VERSION}.bin" ]; then
	wget -q --show-progress -O "${GIT_DIR}/resources/downloads/${HAL9000_ARDUINO_ID}_firmware_${HAL9000_INSTALL_VERSION}.bin" \
